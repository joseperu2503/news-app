...
export default function Searcher({ value,setValue }: {value: string, setValue: (value: string) => void}) {
    ...
    return (
        <div className={`flex bg-white rounded-md py-2 px-4 items-center w-full border-slate-500 border-2 shadow-md`}>
            ...
            />
            {value.length > 0 ? 
                <i aria-hidden className="fas fa-times text-slate-400 cursor-pointer hover:text-slate-700" onClick={event => onChangeValue('')}></i>
            : 
                <i aria-hidden className="fas fa-search text-slate-400"></i>
            }
        </div>
    )
}
